namespace UnityEditor.PostProcessing
{
    ...
    public static class EditorGUIHelper
    {
        ...
        private static Dictionary<string, GUIContent> s_GUIContentCache = new Dictionary<string, GUIContent>();
        ...
        {
            FieldInfo field = ReflectionUtils.GetFieldInfoFromPath(enabledField.serializedObject.targetObject, enabledField.propertyPath);
            object parent = null;
            PropertyInfo prop = null;
            if (field?.IsDefined(typeof(GetSetAttribute), false) ?? false)
            {
                GetSetAttribute attr = (GetSetAttribute)field.GetCustomAttributes(typeof(GetSetAttribute), false)[0];
            ...
            if (e.type == EventType.MouseDown)
            {
                toggleRect.x -= 2.0f;
                toggleRect.y -= 2.0f;
                toggleRect.width += 4.0f;
                toggleRect.height += 4.0f;
